...
配置好中转文件夹后，可以在私聊或已启用的群聊里说“看看中转站里面有什么文件”“列出中转文件夹”，适配器会直接调用飞书 Drive 文件夹清单 API 回复前 20 项，不会把请求交给 Codex/Claude Code 通过浏览器处理。列出后可以继续说“下载第 2 个到桌面”“把第二个保存到桌面”“把中转云盘里的 jpg 文件保存到桌面”；如果先澄清“我说的是那个 jpg 文件”，下一句只说“保存到桌面”也会沿用刚才选中的云空间文件，并先弹下载确认卡片。
