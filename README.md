...
如果上一条 Codex/Claude Code 最终回复里已经给出了本机文件路径，也可以直接回复“发给我”“发一下”。Bridge 会在当前 route 记住最近 10 分钟内最终回复中出现的本地图片/文件候选，先弹“确认发送文件”，不会把这句短回复再交给 Codex/Claude Code 猜。
...
配置好中转文件夹后，可以在私聊或已启用的群聊里说“看看中转站里面有什么文件”“看看中转云盘里面有什么文件”“列出中转文件夹”，适配器会直接调用飞书 Drive 文件夹清单 API 回复前 20 项，不会把请求交给 Codex/Claude Code 通过浏览器处理。列出后可以继续说“下载第 2 个到桌面”“把第二个保存到桌面”“把云盘里的图片保存到桌面”“把中转云盘里的 jpg 文件保存到桌面”；如果先澄清“我说的是那个 jpg 文件”，下一句只说“保存到桌面”也会沿用刚才选中的云空间文件，并先弹下载确认卡片。

当用户说“把云盘里的图片保存到桌面”这类本机下载语义时，Bridge 不会复用前文里本地生成的图片或文件发送给聊天，也不会消耗尚未描述的 pending 附件；该请求需要由飞书 Drive/中转站本地能力处理。

如果需要把手机上传到飞书云空间的普通文件下载到本机，可以把文件链接发给机器人并明确说明下载意图，例如“下载这个文件 <飞书文件链接>”“下载到桌面 <飞书文件链接>”或“下载到 D:\tmp <飞书文件链接>”。下载前会先发确认卡片，只接受发起下载的同一飞书用户确认；确认后文件保存到默认上传目录、桌面或指定的已存在目录。同名文件不会覆盖，会自动追加序号。第一版只支持云空间普通文件，不导出飞书在线文档、表格或多维表格。
