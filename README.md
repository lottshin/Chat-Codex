...
| `/files` | 查看当前 route 最近可引用的文件和飞书云盘项 |
...
可以发送 `/files` 或 `/show files` 查看当前 route 最近可引用的文件。最近文件来源包括 Codex/Claude Code 最终回复里提到的本地交付物、微信/飞书已保存到本机的聊天附件、飞书中转云盘清单项，以及飞书 Drive 下载完成后的本地文件；普通互联网链接不会进入最近文件。

如果上一条 Codex/Claude Code 最终回复里已经给出了本机文件路径，或者 `/files` 里有明确的本地文件候选，也可以直接回复“发给我”“把刚才那个发给我”“发一下”。Bridge 会先弹“确认发送文件”，不会把这句短回复再交给 Codex/Claude Code 猜，也不会绕过审批自动发送。

如果只是“分析这个文件”“看看这个路径”“把网页内容总结后发给我”，Bridge 不会启用文件发送；要求发给别人、邮箱或群聊也不会启用。互联网内容只有在明确要求下载、保存、导出或截图成文件时，才会进入文件发送确认流程。也可以继续使用显式命令：
...
配置方式：在飞书建一个只用于中转权限的群，把机器人拉进群；在云空间建中转文件夹，把文件夹共享给这个群并给可编辑权限；从文件夹 URL 的 `/drive/folder/` 后复制文件夹 token 到 `FEISHU_DRIVE_FOLDER_TOKEN`。也可以在飞书私聊机器人时发送“帮我配置这个中转文件夹”并附上文件夹链接，机器人会保存当前账号的中转文件夹配置并立即生效；如果先只发文件夹链接，机器人会暂存该链接并提示你回复“帮我配置这个中转文件夹”确认。群聊里的链接不会改本机配置。机器人不会直接访问用户个人云盘，Drive 文件上传后只给当前消息事件里的 `open_id` 授予查看权限，再把链接发回当前会话。

配置好中转文件夹后，可以在私聊或已启用的群聊里说“看看中转站里面有什么文件”“看看中转云盘里面有什么文件”“列出中转文件夹”，适配器会直接调用飞书 Drive 文件夹清单 API 回复前 20 项，不会把请求交给 Codex/Claude Code 通过浏览器处理。列出的中转云盘项会进入当前 route 的最近文件历史，`/files` 也能看到。列出后可以继续说“下载第 2 个到桌面”“把第二个保存到桌面”“把云盘里的图片保存到桌面”“把中转云盘里的 jpg 文件保存到桌面”；如果先澄清“我说的是那个 jpg 文件”，下一句只说“保存到桌面”也会沿用刚才选中的云空间文件，并先弹下载确认卡片。

当用户说“把云盘里的图片保存到桌面”这类本机下载语义时，Bridge 不会复用前文里本地生成的图片或文件发送给聊天，也不会消耗尚未描述的 pending 附件；该请求需要由飞书 Drive/中转站本地能力处理。

如果需要把手机上传到飞书云空间的普通文件下载到本机，可以把文件链接发给机器人并明确说明下载意图，例如“下载这个文件 <飞书文件链接>”“下载到桌面 <飞书文件链接>”或“下载到 D:\tmp <飞书文件链接>”。下载前会先发确认卡片，只接受发起下载的同一飞书用户确认；确认后文件保存到默认上传目录、桌面或指定的已存在目录，并记录到最近文件历史。同名文件不会覆盖，会自动追加序号。第一版只支持云空间普通文件，不导出飞书在线文档、表格或多维表格。
