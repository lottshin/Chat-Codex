...
const RECENT_FILE_DELIVERY_PATTERN = /^(?:请\s*)?(?:(?:把|将)\s*)?(?:(?:这个|这张|该|它|刚才(?:(?:那个|那张|那份|的)?(?:图|图片|文件|截图|报告|附件)?)?|上面(?:那个|的)?|上一条(?:里的)?|文件|图片|图|截图|报告|附件)\s*)?(?:发给我|发送给我|传给我|发我|发来|发过来|发一下|发送一下|传一下)(?:\s*(?:吧|谢谢|可以吗)?[。.!！?？]*)?$/i;
