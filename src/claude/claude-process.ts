...
export const CHAT_CLAUDE_BIN_ENV = "CHAT_CLAUDE_BIN";

export type ClaudeBinSource = "default" | "env" | "explicit" | "path";
export type ClaudeWindowsShim = "cmd" | "bat";

export interface ClaudeCommandResolution {
  command: string;
  requested: string;
  ...
  shim?: ClaudeWindowsShim;
}

export interface ResolveClaudeCommandOptions {
  claudeBin?: string;
...
export function resolveClaudeCommand(options: ResolveClaudeCommandOptions = {}): ClaudeCommandResolution {
  const env = options.env ?? process.env;
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const envClaudeBin = env[CHAT_CLAUDE_BIN_ENV]?.trim();
  const requested = (options.claudeBin?.trim() || envClaudeBin || "claude").trim();
  const source: ClaudeBinSource = options.claudeBin?.trim()
    ? "explicit"
    : envClaudeBin
      ? "env"
      : "default";
  ...
  return resolveWindowsClaudeCommand({
    requested,
    source,
    env,
    platform,
...
export function spawnClaude(
  resolution: ClaudeCommandResolution | string,
  args: string[],
  options: SpawnOptions = {},
): ChildProcess {
  const command = typeof resolution === "string" ? resolveClaudeCommand({ claudeBin: resolution }) : resolution;
...
export function formatClaudeCommandSource(source: ClaudeBinSource): string {
  if (source === "env") return CHAT_CLAUDE_BIN_ENV;
  if (source === "path") return "PATH/PATHEXT";
  if (source === "explicit") return "显式参数";
  return "默认 claude";
}

export function formatClaudePlatform(status: { platform?: string; arch?: string } = {}): string {
  return `${status.platform ?? process.platform} ${status.arch ?? process.arch}`;
}

export function formatClaudeCommandForDisplay(command: ClaudeCommandResolution | undefined): string {
  return command?.command ?? "claude";
}

export function formatClaudeUnavailableError(command: ClaudeCommandResolution, error: string): string {
  const lines = [
    error,
    `平台: ${formatClaudePlatform(command)}`,
    `Claude Code CLI: ${command.command}`,
    `来源: ${formatClaudeCommandSource(command.source)}`,
  ];
...
function resolveWindowsClaudeCommand(options: WindowsResolveOptions): ClaudeCommandResolution {
...
function windowsShimKind(command: string): ClaudeWindowsShim | undefined {
...
function spawnWindowsCommandShim(command: ClaudeCommandResolution, args: string[], options: SpawnOptions): ChildProcess {
