...
- 文本包含明确本地路径、桌面/下载目录/当前目录/本地电脑里的文件名，或上下文表达“刚生成的文件/报告/压缩包/图片”等可由后端产出的交付物。
- 互联网内容默认不视为文件发送；只有用户明确说“下载/保存/导出/截图成文件并发给我”时，才按生成型交付物处理。
...
- 日志、报错、搜索结果里出现文件路径。
- 裸 URL、网页链接、网页内容总结或“互联网上的内容发给我”。
- 用户要求发给别人或发到群。
- 表达模糊，无法判断是否授权发送本地文件。

触发后等价于本轮 `/sendfile`：route queue 设置 `sendFile: true`，formatter 追加现有 `BRIDGE_SEND_FILE:` 内部协议说明。最终仍只发送后端在最终回复中声明的文件。

另有一种短句确认路径：如果上一条前台 turn 或后台 Goal 最终回复里已经出现本地图片/文件路径，Bridge 会按 route 记录最近 10 分钟内最多 3 个本地候选。用户随后只说“发给我”“发一下”等短句时，Bridge 直接基于最近候选生成“确认发送文件”卡片，不再把短句交给 Codex/Claude Code；仍然必须点击发送或执行 `/sendfile-approve <编号>` 后才会真正发送。
...
5. 从文件夹 URL 的 `/drive/folder/` 后复制 token 到 `FEISHU_DRIVE_FOLDER_TOKEN`；也可在飞书私聊机器人时发送带配置意图的话术和该文件夹链接，让机器人自动保存当前账号的中转文件夹配置。若先只发送文件夹链接，机器人会暂存该链接并提示用户回复配置确认。

中转文件夹清单：

- 用户说“看看中转站里面有什么文件”“列出中转文件夹”等明确清单意图时，飞书适配器直接调用 Drive 文件夹清单 API。
- 该请求不进入 Codex/Claude Code，避免后端 agent 通过本机浏览器或 Chrome CDP 查看飞书。
- 当前只显示 `FEISHU_DRIVE_FOLDER_TOKEN` 对应文件夹前 20 项，不递归子文件夹。
...
- 自然语言“把桌面上的 a.pptx 发给我”“把下载目录里的账单.xlsx 发来”会启用 sendfile。
- “把这个网页下载成 PDF 发给我”“把这个页面截图发给我”会启用 sendfile。
- 上一条回复提到 `D:\tmp\a.png`，下一条“发给我”会直接弹发送确认，不再启动后端。
- 后台 Goal 最终回复提到本地文件后，下一条“发给我”同样会直接弹发送确认。
- 普通“分析 D:\tmp\a.pdf”不会启用 sendfile。
- “把 https://example.com/a.pdf 发给我”“把网页内容总结后发给我”不会启用 sendfile。
