...
- 普通消息、阶段性输出和最终回复里的路径默认只当文本，不自动发送媒体。用户发送 `/sendfile <任务内容>`，或用明确自然语言要求“把本地文件/桌面或下载目录里的文件/刚生成的文件发给我”时，Bridge 会给该 turn 追加内部协议提示，只在最终回复中解析 `BRIDGE_SEND_FILE: /absolute/path/to/file`，每轮最多发送 3 个文件，并从用户可见最终文本中移除协议行。自然语言分类按“当前用户交付动作 + 本地文件引用或生成/下载/截图型交付物”判断；裸 URL、网页链接和“互联网内容发给我”不会触发，除非用户明确要求下载、保存、导出或截图成文件。若上一条前台 turn 或后台 Goal 最终回复里出现了本地图片/文件路径，用户在同一 route 的 10 分钟内短句回复“发给我”“发一下”时，Bridge 会直接对最近候选生成“确认发送文件”卡片，不再进入 Codex/Claude Code。解析到可发送文件后先生成“确认发送文件”卡片或确认命令文本，只有点击发送或执行 `/sendfile-approve <编号>` 后才会真正发送；取消或未确认都不会发送。自然语言触发只针对当前会话请求者；“分析文件”“查看路径”以及“发给张三/发到群/发邮箱”等第三方外发请求不会启用 sendfile。
- Bridge 维护 route 级最近文件历史，`/files` 和 `/show files` 可查看最近 10 分钟内可引用的本地交付物、已保存聊天附件、飞书中转云盘清单项和飞书 Drive 下载后的本地文件。最近文件历史只记录已经物化为本地文件、聊天附件或飞书云盘文件项的内容；普通互联网链接不进入历史。自然语言引用本地历史项时仍走 sendfile 确认，引用飞书云盘历史项下载到本机时仍走飞书下载确认。
