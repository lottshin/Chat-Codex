...
3. 在云空间建一个中转文件夹。
4. 把文件夹共享给该群，并给可编辑权限。
5. 从文件夹 URL 的 `/drive/folder/` 后复制文件夹 token 到 `FEISHU_DRIVE_FOLDER_TOKEN`；也可以在飞书私聊机器人时发送“帮我配置这个中转文件夹”并附上文件夹链接，由机器人自动写入当前账号的本地凭证并立即生效。如果先只发送文件夹链接，机器人会暂存该链接并提示用户回复“帮我配置这个中转文件夹”确认。群聊里的链接不会改本机配置。

云空间普通文件下载：

- 用户必须明确说“下载”“保存”“下载到桌面”或“下载到某个本地目录”，并附上飞书云空间普通文件链接；只发送文件链接会按普通消息处理。
- 适配器先查询 Drive 元数据并发送确认卡片，确认后才下载到本机。
- 确认卡片只接受发起下载消息的同一 `open_id` 操作，避免其他人触发本机落盘。
- 未指定目录时保存到 `CHAT_CODEX_UPLOAD_DIR` 或默认上传目录；“下载到桌面”保存到当前系统桌面；“下载到 D:\tmp”这类指定目录必须已存在。
- 文件名来自 Drive 元数据并清洗路径字符；同名文件自动追加序号，不覆盖。
- 下载成功后的本地文件会写入当前 route 的最近文件历史，可通过 `/files` 或 `/show files` 查看，也可按本地文件发送规则再次请求发送给聊天。
- 第一版不导出飞书在线文档、表格、多维表格或妙记。

云空间中转文件夹清单：

- 用户明确提到“中转站”“中转云盘”“中转文件夹”“中转目录”等，并询问里面有什么文件、要求查看或列出时，适配器直接调用 `drive.v1.file.list`。
- 清单请求在进入 Codex/Claude Code 前拦截，避免 agent 自行通过本机浏览器、Chrome CDP 或网页会话查看飞书。
- 当前只列出配置的 `FEISHU_DRIVE_FOLDER_TOKEN` 对应文件夹前 20 项，不递归子文件夹。
- 清单结果会写入当前 route 的最近文件历史，序号保持用户刚看到的列表顺序，`/files` 可展示这些飞书云盘项。
- 未配置中转文件夹时直接提示先配置，不把该请求交给后端 agent。

上下文边界：

- 列表后用户说“下载第 2 个到桌面”“把云盘里的图片保存到桌面”“把中转云盘里的 jpg 文件保存到桌面”等下载语义时，适配器优先通过最近文件历史匹配飞书云盘项并发确认卡片，不投递给 Codex/Claude Code。
- 如果云盘下载语义没有被具体渠道消费，Bridge 只回复渠道下载提示；不能复用前文中本地生成的图片/文件发送给聊天，也不能消耗 pending 附件。
